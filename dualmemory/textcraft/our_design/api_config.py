...
def _is_gpt_model_name(model_name: str | None) -> bool:
    name = str(model_name or "").strip().lower()
    return "gpt" in name


def _default_config_path() -> str:
    env_path = os.environ.get("TEXTCRAFT_API_CONFIG_PATH")
    if env_path:
        return env_path
    return os.path.join(os.path.dirname(__file__), "api_config.yaml")


def load_api_config(path: str | None = None, refresh: bool = False) -> dict[str, Any]:
    ...
    if yaml is None:
        raise RuntimeError("PyYAML is required to load TextCraft API config.")
...
def _get_api_section(config: dict[str, Any]) -> dict[str, Any]:
    ...
    return api_section if isinstance(api_section, dict) else {}


def get_api_setting(key: str, default: str = "") -> str:
    config = load_api_config()
    value = _get_api_section(config).get(key)
...
def resolve_api_for_model(
    ...
    *,
    ...
    if api_key and api_base and _looks_like_url(api_key) and not _looks_like_url(api_base):
        warnings.warn(
            "TextCraft api_config: resolved `api_key` looks like a URL and `api_base` does not; swapping them.",
            RuntimeWarning,
        )
...
def apply_api_config(
    ...
    """
    Apply config into environment variables used by our OpenAI client:
    - OPENAI_API_KEY
    - OPENAI_BASE_URL (and OPENAI_API_BASE for compatibility)
    """
