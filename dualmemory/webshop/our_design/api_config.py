...
def _default_config_path() -> str:
    env_path = os.environ.get("WEBSHOP_API_CONFIG_PATH")
    if env_path:
        return env_path
...
def load_api_config(path: Optional[str] = None, refresh: bool = False) -> Dict[str, Any]:
    ...
    if yaml is None:
        raise RuntimeError("PyYAML is required to load WebShop API config.")
...
def resolve_api_for_model(
    ...
    *,
    ...
    if api_key and api_base and _looks_like_url(api_key) and not _looks_like_url(api_base):
        warnings.warn(
            "WebShop api_config: resolved `api_key` looks like a URL and `api_base` does not; "
